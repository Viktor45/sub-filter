[EN](../en/README_en.md) / [RU](../README.md) / [ZH](README_zh.md)
...
✅ 通过 `rules.yaml` 中的[灵活规则](./FILTER_RULES_zh.md) 进行验证  以及来自 `badwords.yaml` 的 [设置](./BADWORDS_zh.md)  
...
- `format` — 输出格式：`yaml`（用于 Clash/Mihomo）、`base64`，或组合如 `yaml+base64`（可选）

**示例：**

- `/filter?id=1` → 过滤第一个订阅
- `/filter?id=1&c=DE` → 仅保留德国服务器
- `/filter?id=1&format=yaml` → 过滤并以 YAML 格式返回（用于 Clash/Mihomo）
- `/filter?id=1&format=base64` → 返回 base64 编码的纯文本
- `/merge?ids=1,2,3&c=US,CA` → 合并三个订阅，仅保留美加服务器
- `/merge?ids=1,2,3&format=yaml` → 合并订阅并以 YAML 格式返回

详细的格式文档请参考：[FORMAT_PARAMETER_zh.md](./FORMAT_PARAMETER_zh.md)
